...
async def decrement_medic_counters():
    db = sqlite3.connect('players.db')
    decrement_value = 2 if not active_pug.active_pug_scheduler.other_pug_enabled else 1
    try:
        c = db.cursor()
...
async def unwarn_player(player: discord.Member, inter: discord.ApplicationCommandInteraction):
    ...
    try:
        ...
        if row is None:
            await inter.send(f"{player_name} has had no recorded warnings. No action taken.")
        elif not row[1]:
            await inter.send(f"{player_name} is not currently warned. No action taken.")
        else:
            c.execute('''UPDATE warnings
            ...
            await inter.send(f"{player_name} has had their warning removed. They now have {row[2] - 1} total warning{'s' if row[2] - 1 != 1 else ''}.")
...
async def player_status(inter: discord.ApplicationCommandInteraction, player: discord.Member):
    ...
    try:
        ...
        if warnings_row is None:
            ...
            pugs_remaining = warnings_row[1] // 2 + (warnings_row[1] % 2 > 0) if not two_pugs else warnings_row[1]
        ...
        if medics_row is None:
            ...
            pugs_remaining = (medics_row[1] // 2) + (medics_row[1] % 2 > 0) if not two_pugs else medics_row[1]
