...
class StartPug:
    ...
    async def list_players(self):
        msg = "Signups in order: " + ', '.join(f"{player.display_name}{f' ({chr(92)}*)' if discord.PartialEmoji.from_str(allclass_emoji_id) in self.player_classes[player] else ''}" for player in self.player_classes.keys())
        return msg
